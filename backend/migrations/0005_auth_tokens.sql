...
CREATE INDEX IF NOT EXISTS idx_users_email_lower ON users (LOWER(email), deleted_at);
...
CREATE INDEX IF NOT EXISTS idx_refresh_tokens_expires_at ON refresh_tokens (expires_at, revoked, deleted_at) WHERE revoked = false;
